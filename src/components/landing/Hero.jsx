...
function Hero() {
  ...
  return (
    <div
      ref={heroRef}
      className="font-karla px-4 sm:px-6 md:px-8 lg:px-16 xl:px-28 flex flex-col items-center text-center pt-20 sm:pt-24 md:pt-20 lg:pt-16 xl:pt-14 bg-background overflow-hidden"
    >
      <BlurIn delay={0.1}>
        <h1 className="font-bold text-3xl sm:text-4xl md:text-5xl lg:text-6xl mt-8 sm:mt-12 md:mt-16 lg:mt-24 text-[#222222] dark:text-white leading-tight">
          Create Meetings with ease
        </h1>
      </BlurIn>

      <BlurIn delay={0.2}>
        <p className="font-normal text-sm sm:text-base md:text-lg mt-3 sm:mt-4 md:mt-5 text-[#666666] dark:text-gray-300 max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl">
          Effortlessly connect with high-quality video and audio. Experience engaging meetings that bring your team closer.
        </p>
      </BlurIn>

      <BlurIn delay={0.3} className="w-full flex justify-center">
        <div className="flex flex-col sm:flex-row mt-4 sm:mt-5 md:mt-6 justify-center gap-2 sm:gap-3 md:gap-4 w-full max-w-md sm:max-w-none">
          <Dialog open={dialogOpen} onOpenChange={handleDialogChange}>
            <DialogTrigger asChild>
              <button
                onClick={startNewMeeting}
                className="bg-[#222222] dark:bg-white dark:text-[#222222] text-white py-2 px-4 sm:py-3 sm:px-8 hover:bg-[#333333] dark:hover:bg-gray-100 hover:text-white rounded-full flex justify-center items-center gap-2 transform-gpu font-semibold w-full sm:w-auto"
              >
          ...
          <form onSubmit={joinMeeting} className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
            <div className="relative w-full sm:w-auto">
              <input
                ...
                className="bg-[#1E1E1E] border border-gray-700 text-white px-6 py-4 rounded-full w-full sm:w-[300px] focus:outline-none focus:border-blue-500 font-light"
              />
              {error && (
            ...
            <button
              type="submit"
              className="bg-[#222222] dark:bg-white text-white dark:text-[#222222] hover:bg-gray-800 dark:hover:bg-gray-200 py-3 px-8 rounded-full font-semibold transition-colors w-full sm:w-auto text-sm sm:text-base"
            >
              Join
            </button>
          </form>
      ...
      <BlurIn delay={0.4} className="w-full max-w-[20rem] sm:max-w-[32rem] md:max-w-[48rem] lg:max-w-[64rem] xl:max-w-[72rem] mt-8 sm:mt-10 md:mt-12 lg:mt-16">
        <img
          src={HeroCover}
          alt="App UI Cover"
      ...
      {showJoinDialog && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-[#1E1E1E] rounded-2xl p-8 max-w-md w-full mx-4">
            <h2 className="text-2xl font-medium mb-4">Log in required</h2>
